...
class DatabaseCliTests(unittest.TestCase):
    def test_parser_supports_import_rebuild_doctor_and_smoke(self):
        parser = build_parser()
        ...
        rebuild_args = parser.parse_args(
            ["rebuild-elasticsearch", "--corpus-dir", "c", "--index-dir", "i"]
        )
        ...
        self.assertEqual(rebuild_args.command, "rebuild-elasticsearch")
        self.assertEqual(rebuild_args.corpus_dir, "c")
        self.assertEqual(rebuild_args.index_dir, "i")
