...
def serialize_message(message: Any) -> dict[str, Any]:
    """Serialize one LangChain message while preserving stream protocol fields."""
    if isinstance(message, dict):
        payload = serialize_lc_object(message)
    ...
    for field in MESSAGE_FIELDS:
        if field in payload and payload[field] is not None:
            result[field] = serialize_lc_object(payload[field])
            continue

        if not isinstance(message, dict) and hasattr(message, field):
            value = getattr(message, field)
            if value is not None:
                result[field] = serialize_lc_object(value)

    if "type" not in result:
        result["type"] = str(getattr(message, "type", type(message).__name__))
...
def serialize_messages_tuple(obj: Any) -> Any:
    ...
    if isinstance(obj, tuple) and len(obj) == 2:
        chunk, metadata = obj
        return [serialize_message(chunk), metadata if isinstance(metadata, dict) else {}]
